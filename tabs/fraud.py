"""Tab 4 — IEEE-CIS Fraud Detection Benchmarks."""
...
RESULTS_PATH = Path(__file__).parent.parent / "results" / "fraud_results.json"
DS = DATASET_INFO["ieee_fraud"]
...
def _leaderboard(df):
    ...
    for i, r in df.iterrows():
        ...
        for c, (fmt, _) in cols.items():
            ...
            val_str = _fmt_inference(r[c]) if c == "Inference (ms/1k)" else fmt.format(r[c])
...
def render():
    ...
    if data is None:
        st.markdown(
            '<div style="text-align:center;padding:4rem;color:#5F6368;">No results yet — run benchmarks/run_fraud.py first.</div>',
    ...
    st.markdown(
        ...
        f'<div class="ds-stat"><div class="ds-stat-val">{data.get("n_features", DS["n_features"])}</div><div class="ds-stat-lbl">Features</div></div>'
        f'<div class="ds-stat"><div class="ds-stat-val">{DS["target_rate"]:.1%}</div><div class="ds-stat-lbl">Fraud Rate</div></div>'
    ...
    c1, _ = st.columns([1, 4])
    with c1:
        mode = st.toggle("Tuned", value=False, help="Zero-effort defaults vs Optuna-tuned", key="fraud_tuned")
    ...
    st.markdown(
        ...
        + mc(
            "Lightest",
            f'{light["Memory (MB)"]:.0f}<span style="font-size:0.9rem;color:#5F6368;">MB</span>',
            light["Model"],
        )
        + "</div>",
        unsafe_allow_html=True,
    ...
    st.markdown(
        '<div class="section-caption">Bottom-right is the sweet spot — accurate AND fast.</div>',
        unsafe_allow_html=True,
    )
    ...
    fig.update_layout(**PLOTLY_LAYOUT, height=460, xaxis_title="Training Time (log scale)", yaxis_title="AUC-ROC")
    ...
    st.markdown(
        '<div class="section-caption">AUC vs training set size — who scales best?</div>',
        unsafe_allow_html=True,
    )
    ...
    if sc_rows:
        ...
        fig2.update_layout(**PLOTLY_LAYOUT, height=420, xaxis_title="Training Samples", yaxis_title="AUC-ROC")
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown('<hr class="soft-divider">', unsafe_allow_html=True)

    st.markdown('<div class="section-header">Raw Data Handling</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-caption">Can it handle messy fraud data out of the box?</div>', unsafe_allow_html=True)
    ...
    if fm_auc >= gb_auc:
        points.append("Foundation models <strong>match or beat</strong> gradient boosting on this setup.")
    else:
        points.append(f"Gradient boosting leads by <strong>{gb_auc - fm_auc:.4f}</strong> AUC.")
    points.append(
        "On highly imbalanced fraud data, tuning usually helps more than on small/clean tabular datasets."
    )
    ...
    with open(RESULTS_PATH) as f:
        st.download_button("Download Results JSON", f.read(), file_name="fraud_results.json", mime="application/json")
