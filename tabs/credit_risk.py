"""Tab 3 — Credit Risk Prediction Benchmarks."""
...
RESULTS_PATH = Path(__file__).parent.parent / "results" / "credit_results.json"
DS = DATASET_INFO["home_credit"]
...
def render():
    data = load_results()
    if data is None:
        st.markdown('<div style="text-align:center;padding:4rem;color:#5F6368;">No results yet — run benchmarks/run_credit.py first.</div>', unsafe_allow_html=True)
        return
    ...
    with c1:
        mode = st.toggle("Tuned", value=False, help="Zero-effort defaults vs Optuna-tuned", key="credit_tuned")
    ...
    with open(RESULTS_PATH) as f:
        st.download_button("Download Results JSON", f.read(), file_name="credit_results.json", mime="application/json")
